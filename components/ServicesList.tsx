...
import { ArrowRight, Globe, Smartphone, LineChart, Cpu, Zap, Layout, Satellite } from 'lucide-react'
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {services.map((service, i) => {
        const Icon = iconMapping[service.icon] || Globe
        return (
          <motion.div
            key={service.slug}
            ...
            <div className="h-48 relative overflow-hidden bg-muted/10">
              {imageMapping[service.slug] ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center">
                  <Icon className="text-accent/20" size={60} />
                </div>
              )}
              <div className="absolute top-4 left-4 w-12 h-12 rounded-xl bg-background/80 backdrop-blur-md flex items-center justify-center border border-border shadow-sm">
